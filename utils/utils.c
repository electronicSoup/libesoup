...
BOOL flash_page_empty(UINT32 address)
{
    ...
    if(   ((address & 0x3f) == 0x00)
       ...
       && (address < LAST_FLASH_ADDRESS)) {
        ...
        serial_log(Debug, TAG, "page_empty() Address Error\n\r");
#endif
    }
    return(FALSE);
}
...
{
#if LOG_LEVEL <= LOG_DEBUG
    serial_log(Debug, TAG, "Flash Erase page %lx\n\r", address);
#endif
    ...
    if(   ((address & 0x3f) == 0x00)
      ...
      serial_log(Error, TAG, "erase_page() Address Error\n\r");
#endif
    }
}
#elif defined(__C30__)
...
void flash_write(UINT32 address, BYTE *data)
{
    ...
    for(i = 0; i < 4; i++) {
        ...
        for (loop = 0; loop < 16; loop++) {
            ...
            serial_log(Debug, TAG, "subStr %s", subStr);
            strcat(string, subStr);
            serial_log(Debug, TAG, "string %s", string);
        }
        serial_log(Debug, TAG, "%s", string);
    }
#endif
#endif
    // Page 64 Byte boundry max address
...
#if LOG_LEVEL <= LOG_DEBUG
        serial_log(Debug, TAG, "Address Error\n\r");
#endif
    }
}
#elif defined(__C30__)
